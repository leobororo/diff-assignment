...
1. The required endpoints host/v1/diff/ID/left, host/v1/diff/ID/right and host/v1/diff/ID provide the diff resource ID. For this reason it was assumed that there is a previous step to create the diff resource (something like an HTTP POST method call that retrieves the newly added diff resource). In order to avoid extra steps to the API usage the calls to host/v1/diff/ID/left and host/v1/diff/ID/right are done through an HTTP PUT call (even though PUT should be used only for updating) but behind the scenes if the diff resource does not exist the application creates a new one. 
2. Because the diff endpoints start with host/v1/diff it was assumed that context-path should not be added to the URLs. For this reason the application endpoints start with http://localhost:8080/v1/diff.
...
1. [Spring Initializr](https://start.spring.io/) was used to bootstrap a Gradle project that uses Spring Boot 2. Spring Data MongoDB, Spring MVC and Spring Boot Test dependencies were also added to it.
...
Since the user is providing diff resource IDs, HTTP PUT method was used to update both left and right texts (host/v1/diff/ID/left, host/v1/diff/ID/right). If the resource does not exist a new resource is created on MongoDB (even though this is supposed to be done with HTTP POST method call in a prior step), otherwise the resource is updated. Before saving two actions are taken:
* If is not possible to decode the text then a DataNotDecodeableException is thrown. The ExceptionControllerHandler (annotated with @ControllerAdvice) intercepts the exception and creates a response with HTTP status equal to 422 - Unprocessable Entity and a body like this:
``` bash
{
...
When the user calls host/v1/diff/ID using GET HTTP method then:
...
```
The asynchronous instance of the application has the "worker" profile activated. When "worker" profile is active the SolveDiffJob bean is managed by the spring container. This bean has a method annotated with @Scheduled that triggers at every 5 seconds and solves the pending diffs:
...
3. Improve the REST API to provide more endpoints. Maybe come up with a better approach to create the resources using HTTP POST method.
4. Understand if storing the files in the MongoDB is the most suitable option.
